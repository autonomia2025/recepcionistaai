...
interface AuthContextType {
  ...
  loading: boolean;
  impersonatedWorkshopId: string | null;
  setImpersonatedWorkshopId: (id: string | null) => void;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [impersonatedWorkshopId, setImpersonatedWorkshopId] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);

  // Use effective profile for the app
  const effectiveProfile = React.useMemo(() => {
    if (!profile) return null;
    if (profile.role === 'SUPERADMIN' && impersonatedWorkshopId) {
      return { ...profile, workshop_id: impersonatedWorkshopId };
    }
    return profile;
  }, [profile, impersonatedWorkshopId]);
  ...
  useEffect(() => {
    // Check for existing impersonation in session storage
    const savedImpersonation = sessionStorage.getItem('impersonatedWorkshopId');
    if (savedImpersonation) {
      setImpersonatedWorkshopId(savedImpersonation);
    }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        if (session?.user) {
          ...
          sessionStorage.removeItem('impersonatedWorkshopId');
        }
      }
    );
  ...
  const handleSetImpersonatedWorkshopId = (id: string | null) => {
    setImpersonatedWorkshopId(id);
    if (id) {
      sessionStorage.setItem('impersonatedWorkshopId', id);
    } else {
      sessionStorage.removeItem('impersonatedWorkshopId');
    }
  };
  ...
  return (
    <AuthContext.Provider
      value={{
        ...
        setImpersonatedWorkshopId: handleSetImpersonatedWorkshopId,
        signIn,
        signUp,
        signOut,
        refreshProfile,
