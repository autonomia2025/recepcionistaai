...
serve(async (req) => {
  ...
  try {
    // ========================================
    // AUTH VALIDATION: Verify user belongs to workshop
    // ========================================
    ...
    if (authHeader?.startsWith('Bearer ')) {
      ...
      if (authData?.user) {
        // Get caller's profile to verify workshop ownership
    ...
    // SECURITY: Verify caller has access to this workshop
    if (!isSuperadmin && callerWorkshopId !== workshop_id) {
      console.error('[verify-whatsapp] Access denied: user workshop mismatch', {
        callerWorkshopId,
        requestedWorkshopId: workshop_id
      });
      return new Response(JSON.stringify({ error: 'Access denied to this workshop' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
