...
serve(async (req) => {
  ...
  try {
    ...
    // 4. Auto-create landing_team entry for the new user
    ...
    const { error: teamError } = await supabase
      ...
      .insert({
        ...
        profile_id: userId,
        name: full_name,
        role: role === 'ADMIN' ? 'Administrador' : 'Profesional',
        show_on_landing: true,
        sort_order: nextSortOrder,
      });

    if (teamError) {
      console.error('Error creating landing_team entry:', teamError);
      // Not critical, continue
    } else {
      console.log('Created landing_team entry for user:', userId);
    }
