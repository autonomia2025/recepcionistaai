...
function validatePhone(phone: string): boolean {
  // Only allow digits, spaces, +, -, (, ) — max 20 chars
  return /^[\d\s+\-()]+$/.test(phone) && phone.length >= 6 && phone.length <= 20;
}
...
async function createGoogleEvent(
  accessToken: string,
  calendarId: string,
  event: { title: string; description?: string; start_time: string; end_time: string; attendees?: { email: string }[] }
): Promise<string | null> {
  try {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // SECURITY: Validate phone format to prevent injection
    ...
    // ========================================
    // SECURITY: Validate professional belongs to workshop
    // ========================================
    const { data: professionalProfile, error: profileError } = await admin
      .from("profiles")
      .select("id, workshop_id, google_calendar_connected, google_refresh_token, google_calendar_id")
      .eq("id", professional_id)
      .single();

    if (profileError || !professionalProfile) {
      console.error("Professional not found:", profileError);
      return jsonResponse(404, { error: "Professional not found" });
    }

    // CRITICAL: Ensure professional belongs to the requested workshop
    if (professionalProfile.workshop_id !== workshop_id) {
      console.error("Security violation: professional does not belong to workshop", {
        professional_id,
        professional_workshop: professionalProfile.workshop_id,
        requested_workshop: workshop_id
      });
      return jsonResponse(403, { error: "Professional does not belong to this workshop" });
    }

    // Find or create contact - use SEPARATE safe queries (no string interpolation)
    const phoneDigits = phone.replace(/\D/g, '');

    // Step 1: Try exact phone match (safe parameterized query)
    ...
    let contactId = exactMatch?.id as string | undefined;

    // Step 2: If no exact match, try fuzzy whatsapp_id match with sanitized digits
    if (!contactId && phoneDigits.length >= 9) {
      const searchPattern = `%${phoneDigits.slice(-9)}%`;
      const { data: fuzzyMatches, error: fuzzyError } = await admin
        .from("contacts")
        .select("id, phone, whatsapp_id")
        .eq("workshop_id", workshop_id)
        .ilike("whatsapp_id", searchPattern)
        .limit(1);

      if (!fuzzyError && fuzzyMatches && fuzzyMatches.length > 0) {
        contactId = fuzzyMatches[0].id;
      }
    }

    console.log("Contact lookup result:", { phone, phoneDigits: phoneDigits.slice(-4), contactId: contactId || 'new' });
    ...
    if (!contactId) {
      ...
      await admin
        ...
        .update({
          ...
          ...(contact_email && { email: contact_email }), // Update email if provided
        })
        .eq("id", contactId);
    ...
    if (professionalProfile?.google_calendar_connected && professionalProfile.google_refresh_token) {
      ...
      if (accessToken) {
        ...
        googleEventId = await createGoogleEvent(accessToken, calendarId, {
          ...
          start_time: start_datetime,
          end_time: end_datetime,
          attendees: contact_email ? [{ email: contact_email }] : []
        });
